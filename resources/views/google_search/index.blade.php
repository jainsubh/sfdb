  ...
  <body>
                ...
                <div class="input-group">
                  <div class="input-group-btn google_search">
                    <form action="" id="google_search_form">
                      ...
                      <input type="text" class="form-control search_box" placeholder="Google Search" id="query" name="search" value="{{ $request->search }}">
                      ...
                      <button class="btn btn-default search_text" id="reset" type="reset" onclick="window.location.href='{{ URL::current() }}'" >Reset</button>
                      -->
                    </form>
                  </div>
                </div>
              ...
              <div class="scroll-wrapper library-body">
                <div class="scroll-inner col-12 col-xs-12" id="google_results">
                  @if(isset($results))
                    ...
                    @endforeach
                    @if($request->has('search') && $request->search != '' && count($paginator) > 0 && $paginator->lastPage() > 1)
                      <article class="panel module google-pagination">
                        <span class="head text">
                          <a href="javascript:void(0)" onclick="search_result(this)" rel="next" aria-label="@lang('pagination.next')">Click here to view more results </a>
                        </span>
                      </article>
                    @endif
                  @endif
    ...
    <script>
      ...
      function search_result(selector){
        ...
        $.ajax({
            url: "{{ route('google_search.search_result', '') }}",
            type: 'GET',
