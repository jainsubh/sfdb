...
<body>
            ...
            <div id="view_as_analyst_popup" class="modal" tabindex="-1" role="dialog" aria-labelledby="analystPopUpLabel">
              <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    ...
                    <form method="POST" action="{{ route('manager.view_as_analyst') }}" enctype="multipart/form-data" id="assign_analyst">
                        {{ csrf_field() }}
                      ...
                      <div class="scroll-inner">
                        <div class="myreports-content">
                          ...
                          <div class="row" id="team_report_content">
                            ...
                            <div class="col-6 col-sm-12 col-xs-12" id="{{ $task->subject_type.'_'.$task->subject_id }}" style="curson:pointer" onclick="show_alert('{{ $task->subject_id }}', '{{ $task->subject_type }}')">
                                <div class="myreports-item">
                                  ...
                                  <p class="date red">Complete Date : {{ \Carbon\Carbon::parse($task->completed_at, 'utc')->setTimezone(auth()->user()->timezone)->isoFormat('lll') }}</p>
                  ...
                  <div class="myreports-inner" id="rew2">
                   ...
                   <div class="myreports-inner" id="rew3">
                    <div class="scroll-wrapper myreports-list">
                      <div class="scroll-inner">
                        <div class="myreports-content">
                          <input type="hidden" id="latest_transfer_request" value="{{ count($task_transfer) > 0 ? \Carbon\Carbon::parse($task_transfer[0]['updated_at'], 'UTC')->timestamp : \Carbon\Carbon::now()->timestamp }}" >
                              ...
                              @foreach($task_transfer as $key => $transfer)
                                ...
                                <div id="{{ $transfer->subject_type }}_{{ $transfer->subject_id }}" class="col-4 col-sm-12 col-xs-12" style="cursor:pointer" onclick="show_alert('<?= $transfer->subject_id ?>', '<?= $transfer->subject_type ?>')">
                                  <div class="myreports-item">
                                    ...
                                    <p class="person blue">Transfer by: {{ $transfer->latest_task_log->assigned_by_user->name }}</p>
                                  </div>
                                </div>
                              ...
                              <div class="alert_pagination row" id="alert_content">
                                @if(!empty($alerts))
                                  @foreach($alerts as $key => $alert)
                                      <div  class="alert-items col-4 col-sm-12 col-xs-12" id="alert_{{$alert->id}}" onclick="show_alert(<?= $alert->id ?>,'alert')" >
  ...
  <script>
    ...
    function reopen(task_id){
      ...
      $.ajax({
          url: "{{ route('tasks.reopen', '') }}/"+task_id,
          type: 'PUT',
          dataType: 'json',
          data: {"_token": "{{ csrf_token() }}"},
          beforeSend: function(){
    ...
    function latest_transfer_request(){
      ...
      $.ajax({
        url: '{{ route("tasks.latest_transfer_request")}}',
        type: 'get',
        data: {
            "updated_at": updated_at
        },
